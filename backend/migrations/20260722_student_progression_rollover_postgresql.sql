...
CREATE TABLE student_progression_mapping_rules (
    ...
    outcome VARCHAR(24) NOT NULL DEFAULT 'CROSS_JENJANG' CHECK(outcome IN ('PROMOTE','RETAIN','GRADUATE','CROSS_JENJANG')),
...
CREATE TABLE student_progression_preview_batches (
    ...
    status VARCHAR(24) NOT NULL DEFAULT 'PREVIEW' CHECK(status IN ('PREVIEW','STALE','COMMITTING','COMMITTED','FAILED','EXPIRED')),
    preview_version INTEGER NOT NULL DEFAULT 1 CHECK(preview_version > 0),
...
CREATE TABLE student_progression_audit (
    ...
    destination_enrollment_id INTEGER REFERENCES student_enrollments(id) ON DELETE RESTRICT,
    student_master_id VARCHAR(36) NOT NULL REFERENCES student_masters(id) ON DELETE RESTRICT,
    outcome VARCHAR(24) NOT NULL CHECK(outcome IN ('PROMOTE','RETAIN','GRADUATE','CROSS_JENJANG','WITHDRAW','EXCLUDE','MANUAL_REVIEW')),
...
CREATE OR REPLACE FUNCTION prevent_student_progression_audit_mutation()
RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'progression audit is append-only'; END; $$ LANGUAGE plpgsql;
CREATE TRIGGER trg_student_progression_audit_no_update BEFORE UPDATE ON student_progression_audit
FOR EACH ROW EXECUTE FUNCTION prevent_student_progression_audit_mutation();
CREATE TRIGGER trg_student_progression_audit_no_delete BEFORE DELETE ON student_progression_audit
FOR EACH ROW EXECUTE FUNCTION prevent_student_progression_audit_mutation();
