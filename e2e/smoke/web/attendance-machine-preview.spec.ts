...
test("@attendance @machine-preview @release previews scan evidence against calendar rules without Alfa inference", async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    for (const weekday of [1, 6]) await save("/api/attendance/calendar/weekday", { academic_year_id: 1, jenjang_id: 1, weekday, expectation: "EXPECTED" });
  ...
  await expect(page.locator("body")).not.toContainText(/Alfa|automatic attendance status|Import attendance/);
});
