...
| --- | ---: | ---: | --- |
| OpenAPI paths | 282 | 203 | 86 unresolved operations are mapped below |
| Operations | 327 | 241 matching operations | 85 migration defects and one deprecated route remain |
| Elysia runtime routes | — | 242 | `/ready` is an Elysia-only readiness route |
...
- 241 FastAPI operations are `PARITY_GREEN` at route and prior-phase evidence
  level.
...
| --- | ---: | --- | --- | --- |
| `api.analytics` | 8 | Dashboard detail, attendance-rate, management-summary, historical-trends, intervention-impact, and both legacy aliases | `MIGRATION_DEFECT` | Filters, lateness aggregates, attendance-rate, monthly-by-class, attendance-report, and intervention-impact are green on both aliases. Implement and replay the remaining paths. |
...
Remaining migration defects: **85**.
