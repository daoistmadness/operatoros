...
Audit commit: Phase 10 student rollback-preview candidate slice.
...
| --- | ---: | ---: | --- |
| OpenAPI paths | 282 | 219 candidate routes | 70 FastAPI operations remain unresolved |
| Operations | 327 | 257 candidate operations | 69 missing operations and one deprecated route remain |
| Elysia runtime routes | — | 258 | `/ready` is an Elysia-only readiness route |
...
| `api.student_import_sessions` | 1 | `POST /api/student-import-sessions/{session_id}/rollback` | `MIGRATION_DEFECT` | Rollback preview is a candidate. Complete rollback commit and dual replay. |
...
Remaining missing operations: **69** plus one deprecated operation. Candidate analytics, operator-queue, teacher-assignment, student-export, roster, staff-import, and student-import-session routes remain parity-blocked until dual replay.
...
The FastAPI and Elysia OpenAPI documents differ because the 99 unresolved
