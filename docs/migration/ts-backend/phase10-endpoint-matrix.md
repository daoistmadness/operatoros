...
| --- | ---: | ---: | --- |
| OpenAPI paths | 282 | 201 | 88 unresolved operations are mapped below |
| Operations | 327 | 239 matching operations | 87 migration defects and one deprecated route remain |
| Elysia runtime routes | — | 240 | `/ready` is an Elysia-only readiness route |
...
- 239 FastAPI operations are `PARITY_GREEN` at route and prior-phase evidence
  level.
...
| --- | ---: | --- | --- | --- |
| `api.analytics` | 10 | Dashboard detail, attendance-rate, management-summary, historical-trends, intervention-impact, and both legacy aliases | `MIGRATION_DEFECT` | Filters, lateness aggregates, attendance-rate, monthly-by-class, and attendance-report are green on both aliases. Implement and replay the remaining paths. |
...
Remaining migration defects: **87**.
