...
| --- | ---: | ---: | --- |
| OpenAPI paths | 282 | 187 | 102 unresolved operations are mapped below |
| Operations | 327 | 225 matching operations | 101 migration defects and one deprecated route remain |
| Elysia runtime routes | — | 226 | `/ready` is an Elysia-only readiness route |
...
- 225 FastAPI operations are `PARITY_GREEN` at route and prior-phase evidence
  level.
...
| --- | ---: | --- | --- | --- |
| `api.analytics` | 24 | Dashboard detail, attendance-rate, management-summary, historical-trends, intervention-impact, and both legacy aliases | `MIGRATION_DEFECT` | Filters are now green on both aliases. Implement and replay the remaining canonical and legacy paths. |
...
Remaining migration defects: **101**.
