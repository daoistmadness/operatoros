...
- **Parity QA Test Coverage:** Always verify changes with the TypeScript report tests in [reports.test.ts](../backend-ts/tests/reports.test.ts). These assertions guarantee numerical alignment between report JSON, PDF content, and editable Excel files.
