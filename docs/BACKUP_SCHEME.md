...
OperatorOS also contains legacy operator shell scripts under `scripts/`. Those scripts use different archive formats and may support PostgreSQL tooling, but they are not equivalent to the authenticated `/api/admin/backups` workflow. In particular, they do not provide the complete database-user attribution, identity-schema viability validation, restored-session revocation, cookie clearing, or browser reauthentication lifecycle.
...
- Backup files are access-controlled but not encrypted by OperatorOS.
