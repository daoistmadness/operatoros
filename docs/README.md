...
- Machine Import Preview (`/attendance/machine-import`) accepts supported attendance-machine `.xlsx` workbooks for preview only. It reports exact identifier matching, scan evidence, and School Calendar expectation separately. Explicit blank source rows remain `NO_SCAN`; omitted source rows are not invented. No attendance status or other business record is changed; no scan is never converted to Alfa.
