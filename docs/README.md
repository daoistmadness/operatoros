...
- Attendance Calendar (`/attendance/calendar`) supports a server-side period preview and explicit apply for inclusive date ranges. It creates only missing date exceptions, treats same-state entries as no-ops, preserves conflicting exceptions, includes weekends, and does not add bulk deletion.
...
- Machine Import (`/attendance/machine-import`) previews supported attendance-machine `.xlsx` workbooks and offers an explicit create-only apply for complete, exact-matched scan pairs on `EXPECTED` dates without existing attendance or overrides. The server re-parses and recomputes the preview digest before applying, writes only canonical `on-time` or `late` attendance, and records provenance in `operations_audit_events`. `NO_SCAN`, duplicate or invalid scans, unmapped identities, calendar blocks, and conflicts are never written; no scan is never converted to Alfa. The preview does not retain the workbook or raw rows. Blocked rows include server-owned resolution guidance and controlled links to the canonical attendance, correction, student, or calendar workflows when the actor can access them.
