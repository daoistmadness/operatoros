...
- **Audit Table Write Abort**: The table `attendance_override_history` is strictly append-only. Any manual `UPDATE` or `DELETE` SQL operations on this table will abort with a database trigger exception (`trg_history_no_update`, `trg_history_no_delete`). To run resets or purges, use the guarded Elysia system operation so triggers are dropped and re-established only inside the approved disposable workflow.
