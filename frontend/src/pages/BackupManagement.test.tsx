...
import BackupManagement,{BackupHistoryPanel,BackupList,BackupStatusPanel,canRestore,formatBytes,handleRestoreReauthentication,RestoreDialog,SchedulerPanel} from "./BackupManagement";
...
vi.mock("lucide-react",()=>{
  ...
  return {AlertTriangle:Icon,ArrowLeft:Icon,CheckCircle2:Icon,DatabaseBackup:Icon,HardDrive:Icon,Loader2:Icon,RefreshCw:Icon,RotateCcw:Icon,ShieldAlert:Icon,X:Icon};
});
...
describe("Backup Management",()=>{
  ...
  it("shows the destructive restore warning",()=>expect(html(<RestoreDialog backup={item} confirmation="" setConfirmation={vi.fn()} destructiveEnabled busy={false} onCancel={vi.fn()} onRestore={vi.fn()}/>)).toContain("pre-restore safety snapshot"));
  it("renders a disabled restore button before matching",()=>expect(html(<RestoreDialog backup={item} confirmation="wrong" setConfirmation={vi.fn()} destructiveEnabled busy={false} onCancel={vi.fn()} onRestore={vi.fn()}/>)).toContain("disabled"));
  it("discloses disabled local settings",()=>expect(html(<RestoreDialog backup={item} confirmation={item.filename} setConfirmation={vi.fn()} destructiveEnabled={false} busy={false} onCancel={vi.fn()} onRestore={vi.fn()}/>)).toContain("disabled by local system settings"));
