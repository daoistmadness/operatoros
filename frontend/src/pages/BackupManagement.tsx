...
import { AlertTriangle, ArrowLeft, CheckCircle2, DatabaseBackup, HardDrive, Loader2, RefreshCw, RotateCcw, ShieldAlert } from "lucide-react";
...
export function BackupList({ backups, onRestore }: { backups: BackupEntry[]; onRestore: (backup: BackupEntry) => void }) {
  ...
  const columns = useMemo(() => [
    ...
    helper.display({ id: "action", header: "Action", enableSorting: false, cell: info => <Button variant="outline" size="sm" onClick={()=>onRestore(info.row.original)} className="border-rose-200 text-rose-700">Restore</Button> }),
  ], [onRestore]);
