...
export default function AttendanceFollowUpDetailModal({ caseId, onClose, onRefresh }) {
  ...
  const [actionType, setActionType] = useState(null); // 'resolve' | 'dismiss' | 'reopen' | null
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail?.message || err?.message || 'Gagal memuat detail kasus follow-up.');
    } finally {
      setLoading(false);
    }
  }, [caseId]);
  ...
  const handleStatusChange = async (targetStatus, extraData = {}) => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail?.message || err?.message || 'Gagal memperbarui status kasus.';
      setError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail?.message || err?.message || 'Gagal menambahkan catatan.');
    } finally {
      setSubmitting(false);
    }
  };
