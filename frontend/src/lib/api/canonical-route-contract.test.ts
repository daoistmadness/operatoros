...
describe('canonical frontend API route contract', () => {
  it('rejects literal backend calls that bypass the /api namespace', () => {
    ...
    for (const file of sourceFiles(sourceRoot)) {
      ...
      for (const match of Array.from(source.matchAll(literalCall))) {
        violations.push(`${path.relative(sourceRoot, file)}:${lineNumber(source, match.index ?? 0)} ${match[2]}`);
      }
      for (const match of Array.from(source.matchAll(literalRequestPath))) {
        violations.push(`${path.relative(sourceRoot, file)}:${lineNumber(source, match.index ?? 0)} ${match[2]}`);
      }
    }
  ...
  it('contains no hardcoded local backend origin or double API prefix', () => {
    ...
    for (const file of sourceFiles(sourceRoot)) {
      const source = fs.readFileSync(file, 'utf8');
      for (const match of Array.from(source.matchAll(forbidden))) {
        violations.push(`${path.relative(sourceRoot, file)}:${lineNumber(source, match.index ?? 0)} ${match[0]}`);
      }
    }
