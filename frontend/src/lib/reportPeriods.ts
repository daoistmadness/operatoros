...
export type ReportTerm = keyof typeof TERM_METADATA;

export function getAcademicYearLabel(anchorYear: number, anchorMonth: number): string {
  const startYear = anchorMonth >= 7 ? anchorYear : anchorYear - 1
  return `TA ${startYear}/${startYear + 1}`
}

export function getTermAcademicYearLabel(term: ReportTerm, year: number): string {
