...
export function managementSummary(context: AuthContext, query: Row): Row {
  ...
  const insights: Row[] = []; if (attendanceSummary.status_percentages.hadir < 95) insights.push({ severity: attendanceSummary.status_percentages.hadir < 90 ? "critical" : "warning", category: "attendance", title: "Kehadiran di Bawah Target", message: `Persentase kehadiran (${attendanceSummary.status_percentages.hadir}%) berada di bawah target minimum 95.0%.`, metric_value: attendanceSummary.status_percentages.hadir, recommended_action: "Tinjau data ketidakhadiran siswa dan tindak lanjuti kasus alfa kronis." }); if (!query.term) insights.push({ severity: "info", category: "data_quality", title: "Laporan Tahunan Penuh", message: "Laporan ini mencakup seluruh tahun ajaran. Gunakan filter Term untuk analisis kuartal yang lebih terfokus.", metric_value: null, recommended_action: "Gunakan menu dropdown filter Term di bagian atas." });
