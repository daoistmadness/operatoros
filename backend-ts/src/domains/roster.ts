...
function rosterCommit(context: AuthContext, body: Row, username: string): Row {
  ...
  const batchRows = JSON.parse(String(batch.rows)) as Row[]; const checksum = digest(batchRows); if (body.preview_checksum && body.preview_checksum !== checksum) throw Object.assign(new Error("Roster preview checksum changed"), { status: 409 });
...
export function rosterRoutes(app: any, context: AuthContext): any {
  ...
  app.post("/api/student-enrollments/roster-preview", async (ctx: Context) => { const user = actor(context, ctx, { capability: "import_student_roster" }); if (!user) return { detail: "Insufficient permissions" }; const file = ctx.body?.file as File | undefined; if (!file || !file.name.toLowerCase().endsWith(".xlsx")) return error(ctx.set, 400, "Academic roster must be an .xlsx workbook"); try { return await rosterPreview(context, file, String(ctx.body.source_owner).trim(), String(ctx.body.date_received), user.username); } catch (cause) { ctx.set.status = 400; return { detail: cause instanceof Error ? cause.message : "The roster workbook could not be previewed." }; } }, { body: t.Object({ file: t.File(), source_owner: t.String({ minLength: 2 }), date_received: t.String() }) });
