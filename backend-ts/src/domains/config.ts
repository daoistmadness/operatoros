...
export function configRoutes(app: any, context: AuthContext, config: { deploymentMode?: string } = {}): any {
  ...
  const kkmUpdateBody = t.Object({ academic_year_id: t.Optional(t.Number({ minimum: 1 })), jenjang_id: t.Optional(t.Number({ minimum: 1 })), subject_id: t.Optional(t.Number({ minimum: 1 })), assessment_type: t.Optional(t.Union([t.Literal("sumatif"), t.Literal("formatif"), t.Literal("overall")])), threshold: t.Optional(t.Number({ minimum: 0, maximum: 100 })) });
  ...
  app.put("/api/academic-config/kkm-thresholds/:threshold_id", (ctx: Context) => { const user = actor(context, ctx, { role: "admin" }); if (!user) return { detail: "Insufficient permissions" }; const client = context.database.client; const before = row(context, "SELECT * FROM kkm_thresholds WHERE id = ?", [ctx.params.threshold_id]); if (!before) return fail(ctx.set, 404, "KKM threshold not found"); const updates = Object.fromEntries(Object.entries(ctx.body).filter(([, value]) => value !== undefined)); const values = { academic_year_id: updates.academic_year_id ?? before.academic_year_id, jenjang_id: updates.jenjang_id ?? before.jenjang_id, subject_id: updates.subject_id ?? before.subject_id, assessment_type: updates.assessment_type ?? before.assessment_type, threshold: updates.threshold ?? before.threshold }; const duplicate = row(context, "SELECT id FROM kkm_thresholds WHERE academic_year_id = ? AND ((jenjang_id = ? AND ? IS NOT NULL) OR (jenjang_id IS NULL AND ? IS NULL)) AND ((subject_id = ? AND ? IS NOT NULL) OR (subject_id IS NULL AND ? IS NULL)) AND assessment_type = ? AND id != ?", [values.academic_year_id, values.jenjang_id, values.jenjang_id, values.jenjang_id, values.subject_id, values.subject_id, values.subject_id, values.assessment_type, before.id]); if (duplicate) return fail(ctx.set, 409, "KKM threshold already exists for this context"); try { client.run("UPDATE kkm_thresholds SET academic_year_id = ?, jenjang_id = ?, subject_id = ?, assessment_type = ?, threshold = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", [values.academic_year_id, values.jenjang_id ?? null, values.subject_id ?? null, values.assessment_type, values.threshold, before.id]); return row(context, "SELECT * FROM kkm_thresholds WHERE id = ?", [before.id]); } catch { return fail(ctx.set, 409, "KKM threshold conflict detected"); } }, { params: t.Object({ threshold_id: t.Number({ minimum: 1 }) }), body: kkmUpdateBody });
