...
describe("student update workbook candidates", () => {
  it("keeps workbook preview non-mutating and rejects invalid rows at commit", async () => {
    ...
    try {
      ...
      const workbook = new ExcelJS.Workbook(); await workbook.xlsx.load(await template.arrayBuffer() as any); const sheet = workbook.getWorksheet("Student Data")!; sheet.addRow(["missing-student", ...Array(headers.length - 1).fill("")]); const bytes = new Uint8Array(await workbook.xlsx.writeBuffer()); const form = new FormData(); form.append("file", new File([bytes], "student-update.xlsx"));
