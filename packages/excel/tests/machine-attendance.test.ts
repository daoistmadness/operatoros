...
describe("machine attendance XLSX preview parser", () => {
  it("keeps scan evidence separate from attendance status and preserves exact identifiers", async () => {
    ...
    expect(result.rows[1]).toMatchObject({ machineStudentIdentifier: "00456", date: "2026-04-04", machineEvidence: "NO_SCAN", scanTimes: [] });
    expect(result.rows[2]).toMatchObject({ machineStudentIdentifier: null, machineEvidence: "INVALID_SCAN_VALUE" });
  });
