...
The launcher starts Vite and Elysia. Vite proxies canonical `/api/*` browser
requests to Elysia.
...
The production bundle is served from `apps/web/build/`.
...
- The default empty value keeps requests same-origin; Vite proxies them during
  development.
- The client sends JSON requests, multipart uploads, and file downloads through the shared request helper in `apps/web/src/lib/api/`.
...
OperatorOS runs with a local Elysia backend and browser-based React UI backed by SQLite. Docker, Nginx, Compose, and PostgreSQL are not runtime dependencies.
