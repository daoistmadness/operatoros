...
export type ManagementReviewProfileFilters = {
  academic_year_id: number; term_id: number; jenjang_id?: number | null; program_id?: number | null; class_id?: number | null;
...
export async function exportManagementReviewAttendance(filters: { academic_year_id: number; term_id: number; jenjang_id: number; program_id?: number | null }) {
  const response = await apiRequest<Blob>({ path: "/api/analytics/management-review/attendance/export.xlsx", params: filters, responseType: "blob", expectedBlobTypes: API_BLOB_TYPES.excel });
  const filename = response.headers["content-disposition"]?.match(/filename="?([^";]+)"?/i)?.[1] ?? "operatoros-management-review-attendance.xlsx";
