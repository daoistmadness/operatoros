...
import { exportManagementReviewAttendance, exportManagementReviewProfile, type ManagementReviewProfileFilters } from "../api/managementReviewProfile";
...
import { useAnalyticsFiltersQuery, useManagementReviewProfileQuery, useManagementReviewProgramsQuery, useManagementReviewTermsQuery } from "../hooks/useAnalyticsQueries";
...
export default function ManagementReviewStudentProfile() {
  ...
  const [search, setSearch] = useSearchParams(); const [exporting, setExporting] = useState<"pdf" | "xlsx" | "attendance" | null>(null); const [exportError, setExportError] = useState("");
  ...
  const academicYearId = numberParam(search, "academic_year_id"); const termId = numberParam(search, "term_id"); const jenjangId = numberParam(search, "jenjang_id"); const programId = numberParam(search, "program_id"); const classId = numberParam(search, "class_id");
  ...
  const termsQuery = useManagementReviewTermsQuery(academicYearId); const classesQuery = useAcademicAnalyticsOptionsQuery(academicYearId, jenjangId, true); const programsQuery = useManagementReviewProgramsQuery(canExport); const terms = termsQuery.data ?? []; const classes = classesQuery.data?.classes ?? [];
  const programs = (programsQuery.data ?? []).filter((value) => jenjangId === null || value.jenjang_id === jenjangId);
  const filters = useMemo<ManagementReviewProfileFilters | null>(() => academicYearId && termId ? { academic_year_id: academicYearId, term_id: termId, jenjang_id: jenjangId, program_id: programId, class_id: classId, residence_group_by: groupBy, residence_top_n: topN } : null, [academicYearId, termId, jenjangId, programId, classId, groupBy, topN]);
  const attendanceExportFilters = useMemo(() => academicYearId && termId && jenjangId ? { academic_year_id: academicYearId, term_id: termId, jenjang_id: jenjangId, program_id: programId } : null, [academicYearId, termId, jenjangId, programId]);
  ...
  useEffect(() => { if (academicYearId && terms.length && !terms.some((term) => term.id === termId)) { const today = new Date().toISOString().slice(0, 10); update({ term_id: (terms.find((term) => term.start_date <= today && term.end_date >= today) ?? terms[0])!.id, jenjang_id: null, program_id: null, class_id: null }); } }, [academicYearId, termId, terms]);
  useEffect(() => { if (classId && !classes.some((value) => value.id === classId)) update({ class_id: null }); }, [classId, classes]);
  useEffect(() => { if (programId && programsQuery.data && !programs.some((value) => value.id === programId)) update({ program_id: null, class_id: null }); }, [programId, programsQuery.data, jenjangId]);
  ...
  const runAttendanceExport = async () => { if (!attendanceExportFilters || classId || programsQuery.error || programsQuery.isPending) return; setExporting("attendance"); setExportError(""); try { const file = await exportManagementReviewAttendance(attendanceExportFilters); download(file.blob, file.filename); } catch { setExportError("Management Review Excel could not be exported. Review the selected scope and try again."); } finally { setExporting(null); } };
  ...
  const actions = <div className="flex flex-wrap gap-2 print:hidden"><Button variant="outline" onClick={reset}><RotateCcw className="mr-2 size-4" />Reset Filters</Button>{canExport && <><Button variant="outline" disabled={!filters || Boolean(exporting)} onClick={() => void runExport("xlsx")}><Download className="mr-2 size-4" />Student Profile Excel</Button><Button variant="outline" disabled={!filters || Boolean(exporting)} onClick={() => void runExport("pdf")}><Download className="mr-2 size-4" />PDF</Button><Button disabled={!attendanceExportFilters || classId !== null || programsQuery.isPending || Boolean(programsQuery.error) || Boolean(exporting)} onClick={() => void runAttendanceExport()}><Download className="mr-2 size-4" />{exporting === "attendance" ? "Exporting…" : programsQuery.isPending ? "Loading programs…" : "Export Management Review Excel"}</Button></>}<Button onClick={() => window.print()} disabled={!reportQuery.data}><Printer className="mr-2 size-4" />Print</Button></div>;
  ...
  return <div className="space-y-7 pb-12 print:p-0"><PageHeader className="lg:flex-col lg:items-stretch" eyebrow="Management Review" title="Student Profile" description="Profil agregat siswa untuk populasi enrollment term yang dipilih." actions={actions} />
    <Card className="print:hidden"><CardHeader><CardTitle>Filter Laporan</CardTitle></CardHeader><CardContent><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-7">
      <div><FieldLabel htmlFor="review-year">Academic Year</FieldLabel><NativeSelect id="review-year" value={academicYearId ?? ""} onChange={(event) => { if (event.target.value !== String(academicYearId ?? "")) update({ academic_year_id: event.target.value, term_id: null, jenjang_id: null, program_id: null, class_id: null }); }}>{years.map((value) => <option key={value.id} value={value.id}>{value.label}</option>)}</NativeSelect></div>
      ...
      <div><FieldLabel htmlFor="review-jenjang">Jenjang</FieldLabel><NativeSelect id="review-jenjang" value={jenjangId ?? ""} onChange={(event) => update({ jenjang_id: event.target.value, program_id: null, class_id: null })}><option value="">Semua</option>{jenjangs.map((value) => <option key={value.id} value={value.id}>{value.name}</option>)}</NativeSelect></div>
      <div><FieldLabel htmlFor="review-program">Academic Program</FieldLabel><NativeSelect id="review-program" value={programId ?? ""} disabled={!canExport || !jenjangId || programsQuery.isPending} onChange={(event) => update({ program_id: event.target.value, class_id: null })}><option value="">All programs</option>{programs.map((value) => <option key={value.id} value={value.id}>{value.name}</option>)}</NativeSelect></div>
      <div><FieldLabel htmlFor="review-class">Class (Student Profile only)</FieldLabel><NativeSelect id="review-class" value={classId ?? ""} onChange={(event) => update({ class_id: event.target.value })}><option value="">Semua</option>{classes.map((value) => <option key={value.id} value={value.id}>{value.name}</option>)}</NativeSelect></div>
      <div><FieldLabel htmlFor="review-group">Tempat Tinggal (Student Profile only)</FieldLabel><NativeSelect id="review-group" value={groupBy} onChange={(event) => update({ residence_group_by: event.target.value })}><option value="kelurahan">Kelurahan</option><option value="kecamatan">Kecamatan</option><option value="city_regency">Kabupaten/Kota</option><option value="province">Provinsi</option></NativeSelect></div>
      <div><FieldLabel htmlFor="review-top">Tampilkan (Student Profile only)</FieldLabel><NativeSelect id="review-top" value={topN} onChange={(event) => update({ residence_top_n: event.target.value })}><option value="5">Top 5</option><option value="10">Top 10</option><option value="all">Semua</option></NativeSelect></div>
    </div></CardContent></Card>
    {canExport && classId !== null && <p className="text-sm text-muted-foreground">Clear the Student Profile Class filter to export the all-class Management Review workbook.</p>}
    {canExport && programsQuery.error && <p role="alert" className="text-sm text-destructive">Academic Program options are unavailable. The export scope cannot be verified.</p>}
