...
function timingLabel(value: string): string {
  return value === "BEFORE_DEADLINE" ? "Before deadline" : value === "DEADLINE_PASSED" ? "Submission deadline passed" : value === "NOT_APPLICABLE" ? "Not applicable" : "Submission deadline unavailable";
}
...
export default function DailyAttendanceOperations() {
  ...
  return <div className="space-y-7">
    <PageHeader eyebrow="Attendance Operations" title="Daily Attendance" description="Review calendar expectation, attendance-recording coverage, and submission timing for one selected date. Not expected means attendance is not required; it does not mark students absent or Alfa." actions={<div className="flex flex-wrap gap-2"><Link className="rounded-md border border-border px-3 py-2 text-sm font-bold hover:bg-surface-muted" to="/attendance/calendar">Attendance Calendar</Link>{can("import_attendance") && <Link className="rounded-md border border-border px-3 py-2 text-sm font-bold hover:bg-surface-muted" to="/upload">Attendance Upload</Link>}{can("view_attendance_corrections") && <Link className="rounded-md border border-border px-3 py-2 text-sm font-bold hover:bg-surface-muted" to="/attendance/override-review">Correction Review</Link>}<Button variant="outline" onClick={reset} className="gap-2"><RotateCcw className="size-4" aria-hidden="true" />Reset filters</Button></div>} />
    ...
    {value.classes.length === 0 ? <EmptyState title="No classes in scope" description="No active classes match the selected filters." /> : <Card><CardHeader><CardTitle>Class coverage</CardTitle><p className="text-sm text-muted-foreground">Expectation, recording coverage, and submission timing are independent server-derived states.</p></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full min-w-[1200px] text-sm"><caption className="sr-only">Daily attendance coverage, expectation, and submission timing by class</caption><thead><tr className="border-b border-border text-left"><th scope="col" className="px-2 py-3">Class</th><th scope="col" className="px-2 py-3">Expectation</th><th scope="col" className="px-2 py-3">Submission</th><th scope="col" className="px-2 py-3">Students</th><th scope="col" className="px-2 py-3">Recorded</th><th scope="col" className="px-2 py-3">Unrecorded</th><th scope="col" className="px-2 py-3">Coverage</th><th scope="col" className="px-2 py-3">Present</th><th scope="col" className="px-2 py-3">Late</th><th scope="col" className="px-2 py-3">Sakit</th><th scope="col" className="px-2 py-3">Izin</th><th scope="col" className="px-2 py-3">Alfa</th><th scope="col" className="px-2 py-3">Actions</th></tr></thead><tbody>{value.classes.map((item) => <tr key={item.classId} className="border-b border-border align-top"><th scope="row" className="px-2 py-3 text-left"><Link className="font-bold text-brand hover:underline" to={classLink(item.classId, value.scope.date)}>{item.className}</Link><span className="block text-xs font-normal text-muted-foreground">{item.jenjang}</span></th><td className="px-2 py-3">{expectationLabel(item.attendanceExpectation.status)}<span className="block text-xs text-muted-foreground">{item.attendanceExpectation.reason === "OUTSIDE_ACADEMIC_YEAR" ? "Outside academic year" : item.attendanceExpectation.reason ? item.attendanceExpectation.reason.replaceAll("_", " ") : item.attendanceExpectation.source === "WEEKDAY_RULE" ? "Weekday rule" : "No configured rule"}</span></td><td className="px-2 py-3"><span>{timingLabel(item.submissionTiming.status)}</span>{item.submissionTiming.deadlineLocalTime && <span className="block text-xs text-muted-foreground">{item.submissionTiming.deadlineLocalTime} Asia/Jakarta</span>}</td><td className="px-2 py-3 tabular-nums">{item.expectedStudentCount}</td><td className="px-2 py-3 tabular-nums">{item.recordedStudentCount}</td><td className="px-2 py-3 tabular-nums">{item.unrecordedStudentCount}</td><td className="px-2 py-3"><span>{stateLabel(item.coverageState)}</span><span className="block text-xs text-muted-foreground">{item.coveragePercent === null ? "Not applicable" : `${item.coveragePercent}%`}</span></td><td className="px-2 py-3 tabular-nums">{item.counts.present}</td><td className="px-2 py-3 tabular-nums">{item.counts.late}</td><td className="px-2 py-3 tabular-nums">{item.counts.sakit}</td><td className="px-2 py-3 tabular-nums">{item.counts.izin}</td><td className="px-2 py-3 tabular-nums">{item.counts.alfa}</td><td className="px-2 py-3"><div className="flex min-w-40 flex-col items-start gap-1">{can("enter_assigned_class_attendance") && <Link className="font-bold text-brand hover:underline" to={entryLink(item.classId, value.scope.date)}>{item.periodFinalized ? "Review attendance" : item.coverageState === "COMPLETE" ? "Review attendance" : "Continue attendance"}</Link>}<Link className="font-bold text-brand hover:underline" to={classLink(item.classId, value.scope.date)}>Open class</Link></div></td></tr>)}</tbody></table></div></CardContent></Card>}
  </div>;
}
