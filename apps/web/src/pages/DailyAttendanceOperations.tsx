...
export default function DailyAttendanceOperations() {
  ...
  return <div className="space-y-7">
    ...
    {value.classes.length === 0 ? <EmptyState title="No classes in scope" description="No active classes match the selected filters." /> : <Card><CardHeader><CardTitle>Class coverage</CardTitle><p className="text-sm text-muted-foreground">These states describe recorded attendance coverage for the selected date.</p></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><caption className="sr-only">Daily attendance recording coverage by class</caption><thead><tr className="border-b border-border text-left"><th scope="col" className="px-2 py-3">Class</th><th scope="col" className="px-2 py-3">Students</th><th scope="col" className="px-2 py-3">Recorded</th><th scope="col" className="px-2 py-3">Unrecorded</th><th scope="col" className="px-2 py-3">Coverage</th><th scope="col" className="px-2 py-3">Present</th><th scope="col" className="px-2 py-3">Late</th><th scope="col" className="px-2 py-3">Sakit</th><th scope="col" className="px-2 py-3">Izin</th><th scope="col" className="px-2 py-3">Alfa</th><th scope="col" className="px-2 py-3">Actions</th></tr></thead><tbody>{value.classes.map((item) => <tr key={item.classId} className="border-b border-border align-top"><th scope="row" className="px-2 py-3 text-left"><Link className="font-bold text-brand hover:underline" to={classLink(item.classId, value.scope.date)}>{item.className}</Link><span className="block text-xs font-normal text-muted-foreground">{item.jenjang}</span></th><td className="px-2 py-3 tabular-nums">{item.expectedStudentCount}</td><td className="px-2 py-3 tabular-nums">{item.recordedStudentCount}</td><td className="px-2 py-3 tabular-nums">{item.unrecordedStudentCount}</td><td className="px-2 py-3"><span>{stateLabel(item.coverageState)}</span><span className="block text-xs text-muted-foreground">{item.coveragePercent === null ? "Not applicable" : `${item.coveragePercent}%`}</span></td><td className="px-2 py-3 tabular-nums">{item.counts.present}</td><td className="px-2 py-3 tabular-nums">{item.counts.late}</td><td className="px-2 py-3 tabular-nums">{item.counts.sakit}</td><td className="px-2 py-3 tabular-nums">{item.counts.izin}</td><td className="px-2 py-3 tabular-nums">{item.counts.alfa}</td><td className="px-2 py-3"><div className="flex min-w-40 flex-col items-start gap-1">{can("enter_assigned_class_attendance") && <Link className="font-bold text-brand hover:underline" to={entryLink(item.classId, value.scope.date)}>{item.periodFinalized ? "Review attendance" : item.coverageState === "COMPLETE" ? "Review attendance" : "Continue attendance"}</Link>}<Link className="font-bold text-brand hover:underline" to={classLink(item.classId, value.scope.date)}>Open class</Link></div></td></tr>)}</tbody></table></div></CardContent></Card>}
  </div>;
}
