...
export default function DailyAttendanceOperations() {
  ...
  return <div className="space-y-7">
    <PageHeader eyebrow="Attendance Operations" title="Daily Attendance" description="Review calendar expectation, attendance-recording coverage, and submission timing for one selected date. Timing is factual and does not judge staff performance." actions={<div className="flex flex-wrap gap-2"><Link className="rounded-md border border-border px-3 py-2 text-sm font-bold hover:bg-surface-muted" to="/attendance/calendar">Attendance Calendar</Link>{can("import_attendance") && <Link className="rounded-md border border-border px-3 py-2 text-sm font-bold hover:bg-surface-muted" to="/attendance/machine-import">Machine Import</Link>}{can("view_attendance_corrections") && <Link className="rounded-md border border-border px-3 py-2 text-sm font-bold hover:bg-surface-muted" to="/attendance/override-review">Correction Review</Link>}<Button variant="outline" onClick={reset} className="gap-2"><RotateCcw className="size-4" aria-hidden="true" />Reset filters</Button></div>} />
