...
import { invalidateAttendanceCalendarQueries, invalidateAttendanceQueries } from "../lib/query/attendanceInvalidation";
...
export default function AttendanceCalendar() {
  ...
  }, [hasUnsavedWeekdays, saveWeekdays.isPending]);
  ...
  return <div className="space-y-7 pb-16">
    ...
    <Card><CardHeader><CardTitle>Calendar scope</CardTitle><p className="text-sm text-muted-foreground">Rules use the school date exactly as entered. Date exceptions override the recurring weekday rule.</p></CardHeader><CardContent><div className="grid gap-4 sm:grid-cols-2"><div><FieldLabel htmlFor="calendar-year">Academic year</FieldLabel><NativeSelect id="calendar-year" value={year.academicYearId} onChange={(event) => { if (!confirmScopeChange()) return; setAcademicYearId(Number(event.target.value)); setJenjangId(null); }}><option value="">Select year</option>{years.data?.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</NativeSelect><p className="mt-1 text-xs text-muted-foreground">{year.startDate} to {year.endDate}</p></div><div><FieldLabel htmlFor="calendar-jenjang">Jenjang</FieldLabel><NativeSelect id="calendar-jenjang" value={current.id} onChange={(event) => { if (!confirmScopeChange()) return; setJenjangId(Number(event.target.value)); }}>{calendar.data.jenjangs.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</NativeSelect></div></div></CardContent></Card>
    ...
    <Card><CardHeader><CardTitle id="recurring-weekdays">Recurring weekdays · {current.name}</CardTitle><p className="text-sm text-muted-foreground">Changes are not active until saved. Not configured resolves to UNKNOWN.</p></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{weekdayOrder.map((weekday) => {
